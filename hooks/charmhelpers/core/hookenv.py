...
def status_get(include_data=False, service=False):
    """Retrieve the previously set juju workload state

    If the status-set command is not found then assume this is juju < 1.23 and
    return 'unknown'

    If service return status for all units of this service if this unit is the leader

    If include_data is set return dictionary
    """
    cmd = ['status-get']
    if service:
        cmd.append('--service')
...
def status_compound(current_status, workload_state, message):
    """Compound statuses: Take the current status and new
    workload state and message and status_set with the correct
    workload state and a compound message
    """
    hierarchy = {"unknown": -1,
                 "active": 0,
                 "maintenance": 1,
    ...
    # Set workload_state based on heirarchy of statuses
